        ...
        try {
            if (currState === 'Sign Up') {
                ...
                if (response.data && response.data.requiresOtp) {
                    toast.info(response.data.devOtp ? `OTP: ${response.data.devOtp}` : "OTP sent to your email!");
                    setCurrState('LoginOTP');
                } else {
                    toast.success("Login Successful!");
                    localStorage.setItem('adminToken', response.data.token);
                    localStorage.setItem('adminName', response.data.name);
            ...
            if (error.response && error.response.data) {
                const msg = typeof error.response.data === 'string' ? error.response.data : error.response.data.message || "An error occurred";
                toast.error(msg);
            } else {
        ...
        <div className="login-container">
            <div className="login-card">
                ...
                <form className="login-form" onSubmit={handleSubmit}>
                    ...
                    {currState !== 'OTP' && currState !== 'LoginOTP' && (
                        <>
                            <div className="form-group floating">
                    ...
                    <button type="submit" className="login-btn">
                        {currState === 'Sign Up' ? 'Request OTP' : (currState === 'OTP' || currState === 'LoginOTP') ? 'Verify OTP' : 'Login'}
                    </button>
